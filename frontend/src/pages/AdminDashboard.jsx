...
export default function AdminDashboard() {
  ...
  const approveOwner = (id) =>
    setOwners(owners.map((o) => (o.id === id ? { ...o, approved: true } : o)));

  const toggleLotStatus = (id) =>
    setOwners(
      owners.map((o) =>
        o.id === id ? { ...o, lot: { ...o.lot, active: !o.lot.active } } : o,
      ),
    );

  return (
    <div
      className="min-h-screen p-8 space-y-16 transition-colors duration-300"
      style={{
    ...
    >
          ...
          {users
            ...
            .map((u) => (
              <tr
                ...
                style={{ backgroundColor: darkMode ? "#8fd1da" : "#FFFFFF" }}
                onMouseEnter={(e) =>
              ...
              >
                <TD style={{ color: darkMode ? "#0b0c0c" : "#0B2E33" }}>
                  {u.name}
                </TD>
                <TD style={{ color: darkMode ? "#0f1414" : "#0B2E33" }}>
                  {u.email}
                </TD>
                <TD style={{ color: darkMode ? "#0f1414" : "#0B2E33" }}>
                  {u.vehicle}
      ...
      <Section title="Booking & Payment Oversight">
        <Table headers={["User", "Parking", "Amount", "Status"]}>
          {bookings.map((b) => (
            ...
            >
              <TD style={{ color: darkMode ? "#E6F6F8" : "#0B2E33" }}>
                {b.user}
              </TD>
              <TD style={{ color: darkMode ? "#E6F6F8" : "#0B2E33" }}>
                {b.lot}
      ...
      <Section title="System Settings">
        <div
          className="max-w-md space-y-5 p-6 rounded-2xl shadow-md"
          style={{ backgroundColor: "#b8E3E9" }}
        >
...
function KPI({ title, value }) {
  return (
    <div
      className="p-6 rounded-2xl shadow-md hover:shadow-lg transition cursor-pointer"
      style={{ backgroundColor: "#b8E3E9" }}
    >
      <p className="text-sm font-medium" style={{ color: "#0B2E33" }}>
        {title}
      </p>
      <h3 className="text-3xl font-bold mt-1" style={{ color: "#4F7C82" }}>
        {value}
      </h3>
    </div>
  );
